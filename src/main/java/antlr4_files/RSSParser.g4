...
height      :   O_HEIGHT    INT C_HEIGHT
            { if (Integer.parseInt($INT.text) > 400) {
                throw new RuntimeException("height " + $INT.text + " no debe ser mayor a 400.");
            }}
            ;

width       :   O_WIDTH     INT C_WIDTH
            { if (Integer.parseInt($INT.text) > 144) {
                throw new RuntimeException("width " + $INT.text + " no debe ser mayor a 144.");
            }}
            ;
